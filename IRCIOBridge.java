package com.github.thedaemoness
...
public class IRCIOBridge implements AutoCloseable {
	...
	private static class Output extends OutputStream {
		...
		public Output(OutputStream sockOut, String chan) {
			output = sockOut;
	...
	private static class Input extends InputStream {
		...
		public Input(Scanner input, Writer output, String chan) throws IOException {
			this.input = input;
			this.output = output;
			this.chan = chan;
		}
	...
	public IRCIOBridge(
		...
		Scanner input = new Scanner(s.getInputStream(), "UTF8");
		...
		this.in = new Input(input, writer, CHANNEL);
	}
	public IRCIOBridge(
		String server,
		String channel,
	...
	public static IRCIOBridge build(String server, String channel, String... nicks)
	throws UnknownHostException, IOException {
		final IRCIOBridge irccc = new IRCIOBridge(server, channel, nicks);
		return irccc;
	}
}
